// Converter tool from GoatTracker2 song format to minimal player v2
// Cadaver (loorni@gmail.com) 9/2021
...
#define MP_LASTNOTE 0x78
#define MP_REST 0x7a
#define MP_WAVEPOS 0x7b
#define MP_SETWAVE 0x7c
#define MP_SETAD 0x7d
#define MP_SETSR 0x7e
#define MP_NOCMD 0x1
...
unsigned char mpwavesrtbl[MAX_MPTBLLEN+1];
...
void clearmpsong(void)
{
    ...
    memset(mpwavesrtbl, 0, sizeof mpwavesrtbl);
...
void convertsong(void)
{
    ...
    memset(waveusedmap, 0, sizeof waveusedmap);
    ...
    for (e = 1; e <= highestusedinstr; e++)
    {
        ...
        mpinssr[mpinssize] = instr[e].sr ? instr[e].sr : 0x01;
    ...
    for (e = 0; e <= highestusedpatt; e++)
    {
        ...
        for (c = 0; c < MAX_PATTROWS*2; c++)
        {
            if (notecolumn[c] == MP_ENDPATT)
                break;
            if (durcolumn[c] > 0 && durcolumn[c] < 2)
            {
                int orig = durcolumn[c];
                int inc = 2-durcolumn[c];
                durcolumn[c] += inc;
                durcolumn[c+1] -= inc;
        ...
        // Adjust the last duration before endpatt
        for (c = 0; c < MAX_PATTROWS*2; c++)
        {
            if (notecolumn[c] == MP_ENDPATT)
            {
        ...
        for (c = 0; c < MAX_PATTROWS*2; c++)
        {
            if (notecolumn[c] == MP_ENDPATT)
                break;
            // Always leave the final adjusted duration so that it can be re-adjusted during pattern merging
            if (c && durcolumn[c] == lastdur && c != lastdurrow)
                durcolumn[c] = 0;
            if (durcolumn[c])
                lastdur = durcolumn[c];
        }
        ...
        for (c = 0; c < MAX_PATTROWS*2; c++)
        {
            ...
            if (durcolumn[c])
            {
                mppattlastdurindex[e] = pattlen;
                mppatterns[e][pattlen++] = 0x100 - durcolumn[c];
            }
    ...
    do
    {
        ...
        for (e = 1; e <= highestusedpatt+1; ++e)
        {
            if (mppattnext[e] > 0 && mppattnext[e] < 128 && mppattprev[mppattnext[e]] == e && mppattlen[e-1] + mppattlen[mppattnext[e]-1] <= 256)
            {
                ...
                // Remove the end pattern duration adjust
                mppatterns[mergedest][mppattlastdurindex[mergedest]]--;
                memcpy(&mppatterns[mergedest][mppattlen[mergedest]-1], &mppatterns[mergesrc][0], mppattlen[mergesrc]);
                mppattlastdurindex[mergedest] = mppattlen[mergedest]-1 + mppattlastdurindex[mergesrc];
                ...
                for (f = mergesrc; f < highestusedpatt; ++f)
                {
                    ...
                    mppattlastdurindex[f] = mppattlastdurindex[f+1];
                }
                --highestusedpatt;
                ...
                for (f = 0; f <= highestusedsong; ++f)
                {
                    for (g = 0; g < mpsongtotallen[f];)
                    {
                        if (mptracks[f][g] == mergesrc+1)
                        {
                            ...
                            {
                                if (mptracks[f][h] == 0 && (h == 0 || mptracks[f][h-1] != 0) && mptracks[f][h+1] > g)
                                    --mptracks[f][h+1];
                            }
                        }
                        else if (mptracks[f][g] == 0 && (g == 0 || mptracks[f][g-1] != 0))
                        {
    ...
    // Add unique wavetable steps for SR init
    printf("Adding unique wavetable steps for ADSR init\n");
    for (e = 0; e < mphrinssize; ++e)
    {
        mpwavesrtbl[mpwavesize+e] = mpinssr[e];
        mpwavetbl[mpwavesize+e] = mpwavetbl[mpinswavepos[e]-1];
        mpnotetbl[mpwavesize+e] = mpnotetbl[mpinswavepos[e]-1];
        mpwavenexttbl[mpwavesize+e] = mpwavenexttbl[mpinswavepos[e]-1];
        mpinswavepos[e] = mpwavesize+e+1;
    }
    mpwavesize += mphrinssize;

    printf("Removing unused wavetable steps and optimizing\n");
    for (e = 0; e < 256; ++e)
        waveposmap[e] = 0;
    for (e = 0; e < mpinssize; ++e)
        checkwaveused(mpinswavepos[e]);
    for (e = 0; e <= highestusedpatt; e++)
    {
        for (f = 0; f < mppattlen[e]; ++f)
        {
            if (mppatterns[e][f] == 0)
                break;
            if (mppatterns[e][f] >= 0x80 || mppatterns[e][f] == MP_REST || (mppatterns[e][f] < MP_REST && (mppatterns[e][f] & MP_NOCMD)))
                continue;
            if (mppatterns[e][f] == MP_WAVEPOS)
                checkwaveused(mppatterns[e][f+1]);
...

    {
        ...
        unsigned char newwavenexttbl[256];
        unsigned char newwavesrtbl[256];
        int newwavesize = 0;

        // Instrument inits first
        for (e = 0; e < mpwavesize; ++e)
        {
            if (!waveusedmap[e+1] || !mpwavesrtbl[e])
                continue;

            newwavetbl[newwavesize] = mpwavetbl[e];
            newnotetbl[newwavesize] = mpnotetbl[e];
            newwavenexttbl[newwavesize] = mpwavenexttbl[e];
            newwavesrtbl[newwavesize] = mpwavesrtbl[e];
            waveposmap[e+1] = newwavesize+1;
            ++newwavesize;
        }
        // Then others with waveforms
        for (e = 0; e < mpwavesize; ++e)
        {
            if (!waveusedmap[e+1])
                continue;

            if (!mpwavesrtbl[e] && mpwavetbl[e] >= 0x10 && mpwavetbl[e] < 0x90)
            {
                newwavetbl[newwavesize] = mpwavetbl[e];
                newnotetbl[newwavesize] = mpnotetbl[e];
                newwavenexttbl[newwavesize] = mpwavenexttbl[e];
        ...
        // Finally vibratos/slides
        for (e = 0; e < mpwavesize; ++e)
        {
            if (!waveusedmap[e+1])
                continue;

            if (mpwavetbl[e] < 0x10 || mpwavetbl[e] >= 0x90)
            {
                newwavetbl[newwavesize] = mpwavetbl[e];
                newnotetbl[newwavesize] = mpnotetbl[e];
                newwavenexttbl[newwavesize] = mpwavenexttbl[e];
        ...
        for (e = 0; e < newwavesize; ++e)
        {
            if (newwavetbl[e] != 0 && newwavetbl[e] != 0x90)
                newwavenexttbl[e] = waveposmap[newwavenexttbl[e]];
        }
        
        for (e = 0; e < mpinssize; ++e)
            mpinswavepos[e] = waveposmap[mpinswavepos[e]];
        ...
        for (e = 0; e <= highestusedpatt; e++)
        {
            for (f = 0; f < mppattlen[e]; ++f)
            {
                ...
                if (mppatterns[e][f] >= 0x80 || mppatterns[e][f] == MP_REST || (mppatterns[e][f] < MP_REST && (mppatterns[e][f] & MP_NOCMD)))
                    continue;
                if (mppatterns[e][f] == MP_WAVEPOS)
                    mppatterns[e][f+1] = waveposmap[mppatterns[e][f+1]];

                ++f;
            }
        }

        memcpy(mpwavetbl, newwavetbl, sizeof mpwavetbl);
        memcpy(mpnotetbl, newnotetbl, sizeof mpnotetbl);
        memcpy(mpwavenexttbl, newwavenexttbl, sizeof mpwavenexttbl);
        memcpy(mpwavesrtbl, newwavesrtbl, sizeof mpwavesrtbl);

        mpwavesize = newwavesize;
    }
...
void checkwaveused(unsigned char pos)
{
    for (;;)
    {
        unsigned char lastpos = pos;
        if (!pos)
            break;

        waveusedmap[pos] = 1;

        if (mpwavetbl[pos-1] == 0 || mpwavetbl[pos-1] == 0x90)
            break;

        pos = mpwavenexttbl[pos-1];
        if (pos == lastpos)
            break;
        if (waveusedmap[pos])
            break;
...
void savempsong(const char* songfilename)
{
    int c, d;
    int totalsize = 0;
    int allocbeforepatt = 0;
    int alldone = 0;
    int pattpage[256];
    int pagefree[256];
    int pattbegin[256];
    int bestfit, bestfitpage;
    int firstpattpage, lastpattpage;
    ...
    totalsize += mpwaveadsrsize;
    totalsize += mppulsesize * 3;
    totalsize += mpfiltsize * 3;
    for (c = 0; c <= highestusedsong; ++c)
        totalsize += mpsongtotallen[c];

    allocbeforepatt = totalsize;
    for (c = 0; c < 256; ++c)
    {
        int allocnow = allocbeforepatt > 255 ? 256 : allocbeforepatt;
        if (!allocnow)
            break;
        pagefree[c] = 256 - allocnow;
        allocbeforepatt -= allocnow;
    }
    for (c = 0; c < 256; ++c)
    {
        if (pagefree[c] > 0)
    ...
    if (startaddress >= 0)
    {
        // Pattern data requires no pagecrosses, start on a page
        startaddress &= 0xff00;
        fprintf(out, "  org $%04x\n", startaddress);
    ...
    fprintf(out, "%spattTblLo:\n", prefix);
    for (c = 0; c <= highestusedpatt; ++c)
        fprintf(out, "  dc.b <%spatt%d\n", prefix, c);
    fprintf(out, "\n");
    fprintf(out, "%spattTblHi:\n", prefix);
    for (c = 0; c <= highestusedpatt; ++c)
        fprintf(out, "  dc.b >%spatt%d\n", prefix, c);
    fprintf(out, "\n");
    ...
    writeblock(out, "waveSRTbl", mpwavesrtbl, mpwaveadsrsize);
    ...
    for (d = firstpattpage; d <= lastpattpage; ++d)
    {
        for (c = 0; c <= highestusedpatt; ++c)
        {
            if (pattpage[c] == d)
            {
                fprintf(out, "; Pattern %d on page %d ofs %d\n", c, pattpage[c], pattbegin[c]);
                char namebuf[80];
        ...
        {
            fprintf(out, "; %d bytes padding at the end of page %d\n", pagefree[d], d);
            char namebuf[80];
            sprintf(namebuf, "pageFill%d", d);
            unsigned char empty[256];
            memset(empty, 0, sizeof empty);
            writeblock(out, namebuf, &empty[0], pagefree[d]);
        }
    }
}
